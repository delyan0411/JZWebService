        ...
        public void Start(object source, System.Timers.ElapsedEventArgs e)
        {
            ...
            for (int i = 0; i < flow141.Count; i++)
            {
                ...
                if (string.IsNullOrEmpty(tmp))
                {
                    wsretlog = "日期为空";
                    errtype = 3;
                    MySqlHelper.ExecuteNonQuery(System.Data.CommandType.Text, string.Format("UPDATE FLOW_RUN set TIMES=TIMES+1,RETRY=0 where RUN_ID={0} ", flow141[i].run_id), null);
                    MySqlHelper.ExecuteNonQuery(System.Data.CommandType.Text, string.Format("INSERT INTO sync_flowlog (run_id,flow_id,run_name,sendlog,receivelog,sendtime,errtype) values ({0},{1},'{2}','{3}','{4}',sysdate(),{5}) ", flow141[i].run_id, flowid, flow141[i].run_name, "", wsretlog, errtype), null);
                    continue;
                }
                if (!string.IsNullOrEmpty(tmp))
                {
                        ...
                        if (!string.IsNullOrEmpty(tmpnewline[w]))
                        {
                            ...
                            if (oa13saplist.Count > 0)
                            {
                                ...
                                try
                                {
                                    ...
                                    Logger.Log(flow141[i].run_id + "(" + flow141[i].run_name + "-" + flow141[i].user_id + "):" + JsonHelper.ObjectToJson(oa13saplist), flowid.ToString());
                                    WebReference.DT_OA_OA13_RespITEM[] retoa = oa.SAP_OA_JK_13(oa13saplist.ToArray());
                                    wsretlog = JsonHelper.ObjectToJson(retoa);
                                    Logger.Log(flow141[i].run_id + "(" + flow141[i].run_name + "-" + flow141[i].user_id + "):" + JsonHelper.ObjectToJson(retoa), flowid.ToString());
                                ...
                                MySqlHelper.ExecuteNonQuery(System.Data.CommandType.Text, string.Format("UPDATE FLOW_RUN set TIMES=TIMES+1,RETRY=0 where RUN_ID={0} ", flow141[i].run_id), null);
                                MySqlHelper.ExecuteNonQuery(System.Data.CommandType.Text, string.Format(" INSERT INTO sync_flowlog (run_id,flow_id,run_name,sendlog,receivelog,sendtime,errtype) values ({0},{1},'{2}','{3}','{4}',sysdate(),{5})", flow141[i].run_id, flowid, flow141[i].run_name, JsonHelper.ObjectToJson(oa13saplist), wsretlog, errtype), null);
                                if (ifsuc)
                                {
                                    int ret = MySqlHelper.ExecuteNonQuery(System.Data.CommandType.Text, string.Format("update flow_run set SYNC_TIME=sysdate(),RETRY=0 where RUN_ID={0} ", flow141[i].run_id), null);
