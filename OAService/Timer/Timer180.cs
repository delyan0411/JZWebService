...
{
    //集团中心办公室人员请假
    public class Timer180
    {
        public Timer180()
        {
            System.Timers.Timer t = new System.Timers.Timer(Convert.ToInt32(System.Configuration.ConfigurationManager.AppSettings["timer180"])* 1000);//
        ...
        public void Start(object source, System.Timers.ElapsedEventArgs e)
        {
            ...
            List<Flow180> flow180 = MySqlHelper.ExecuteObjectList<Flow180>(string.Format("SELECT flowr.run_id AS run_id,flowr.run_name AS run_name,byname AS user_id,data_78 AS DATA,data_90 AS i_awart FROM flow_data_{0} AS flowdata INNER JOIN flow_run AS flowr ON flowdata.run_id = flowr.run_id INNER JOIN USER AS u ON u.USER_ID = flowdata.begin_user WHERE FLOW_ID = {0} AND DEL_FLAG = 0 AND data_78!=\"\" AND END_TIME IS NOT NULL AND SYNC_TIME IS NULL AND (TIMES<=2 or RETRY=1)", flowid)).ToList();
            for (int i = 0; i < flow180.Count; i++)
            {
                string wsretlog = "";
                ...
                string tmp = flow180[i].data.ToString();
                string awart = "";
                if (Enum.IsDefined(typeof(I_awartType), flow180[i].i_awart))
                {
                    foreach (string awartType in Enum.GetNames(typeof(I_awartType)))
                    {
                        if (flow180[i].i_awart == awartType)
                        {
                ...
                if (string.IsNullOrEmpty(tmp))
                {
                    wsretlog = "日期为空";
                    errtype = 3;
                    MySqlHelper.ExecuteNonQuery(System.Data.CommandType.Text, string.Format("UPDATE FLOW_RUN set TIMES=TIMES+1,RETRY=0 where RUN_ID={0} ", flow180[i].run_id), null);
                    MySqlHelper.ExecuteNonQuery(System.Data.CommandType.Text, string.Format("INSERT INTO sync_flowlog (run_id,flow_id,run_name,sendlog,receivelog,sendtime,errtype) values ({0},{1},'{2}','{3}','{4}',sysdate(),{5}) ", flow180[i].run_id, flowid, flow180[i].run_name, "", wsretlog, errtype), null);
                    continue;
                }
                if (!string.IsNullOrEmpty(tmp))
                {
                                ...
                                if (endtime.CompareTo(starttime) > 0)
                                {
                                    ...
                                    if (enddate.CompareTo(startdate) > 0)
                                    {
                                        ...
                                        for (int d = 0; d <= days; d++)
                                        {
                                            ...
                                            oa13sap.I_PERNR = flow180[i].user_id.ToString();
                                            // if (days > 0)
                                            //{
                    ...
                    if (oa13saplist.Count > 0)
                    {
                        ...
                        try
                        {
                            ...
                            Logger.Log(flow180[i].run_id + "(" + flow180[i].run_name + "-" + flow180[i].user_id + "):" + JsonHelper.ObjectToJson(oa13saplist), flowid.ToString());
                            WebReference.DT_OA_OA13_RespITEM[] retoa = oa.SAP_OA_JK_13(oa13saplist.ToArray());
                            wsretlog = JsonHelper.ObjectToJson(retoa);
                            Logger.Log(flow180[i].run_id + "(" + flow180[i].run_name + "-" + flow180[i].user_id + "):" + JsonHelper.ObjectToJson(retoa), flowid.ToString());
                        ...
                        MySqlHelper.ExecuteNonQuery(System.Data.CommandType.Text, string.Format("UPDATE FLOW_RUN set TIMES=TIMES+1,RETRY=0  where RUN_ID={0} ", flow180[i].run_id), null);
                        MySqlHelper.ExecuteNonQuery(System.Data.CommandType.Text, string.Format("  INSERT INTO sync_flowlog (run_id,flow_id,run_name,sendlog,receivelog,sendtime,errtype) values ({0},{1},'{2}','{3}','{4}',sysdate(),{5}) ", flow180[i].run_id, flowid, flow180[i].run_name, JsonHelper.ObjectToJson(oa13saplist), wsretlog, errtype), null);
                        if (ifsuc)
                        {
                            int ret = MySqlHelper.ExecuteNonQuery(System.Data.CommandType.Text, string.Format("update flow_run set SYNC_TIME=sysdate(),RETRY=0  where RUN_ID={0} ", flow180[i].run_id), null);
                            //更新数据库表示他已经同步过了并且同步成功了
                            if (ret > 0)
                                Logger.Log(string.Format("RUN_ID={0},已经更新数据库", flow180[i].run_id), flowid.ToString());
                            else
                                Logger.Error(string.Format("RUN_ID={0},更新没有成功", flow180[i].run_id), flowid.ToString());
                        }
                        else
                        {
                            Logger.Log(string.Format("RUN_ID={0},SAP返回错误信息", flow180[i].run_id), flowid.ToString());
                        }
                    }
                }
            }
