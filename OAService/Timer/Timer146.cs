            ...
            for (int i = 0; i < flow146.Count; i++)
            {
                ...
                if (!string.IsNullOrEmpty(st) && !string.IsNullOrEmpty(et))
                {
                    ...
                    if (endtime.CompareTo(starttime) > 0)
                    {
                        ...
                        MySqlHelper.ExecuteNonQuery(System.Data.CommandType.Text, string.Format("UPDATE FLOW_RUN set TIMES=TIMES+1 where RUN_ID={0} ", flow146[i].run_id), null);
                        MySqlHelper.ExecuteNonQuery(System.Data.CommandType.Text, string.Format(" INSERT INTO sync_flowlog (run_id,flow_id,run_name,sendlog,receivelog,sendtime) values ({0},{1},'{2}','{3}','{4}',sysdate()) ", flow146[i].run_id, flowid, flow146[i].run_name, JsonHelper.ObjectToJson(oasap17), retoastring), null);
                        if (ifsuc)
                        {
                            int ret = MySqlHelper.ExecuteNonQuery(System.Data.CommandType.Text, string.Format("update flow_run set SYNC_TIME=sysdate() where RUN_ID={0} ", flow146[i].run_id), null);
