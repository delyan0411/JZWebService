...
{
    //九洲大药房_SAP_HR_PT_150_门店人员加班审批流程
    public class Timer149
    {
        public Timer149()
        {
            System.Timers.Timer t = new System.Timers.Timer(Convert.ToInt32(System.Configuration.ConfigurationManager.AppSettings["timer149"])* 1000);//
        ...
        public void Start(object source, System.Timers.ElapsedEventArgs e)
        {
            ...
            for (int i = 0; i < flow149.Count; i++)
            {
                ...
                string st = flow149[i].starttime;
                string et = flow149[i].endtime;
                if (string.IsNullOrEmpty(st) || string.IsNullOrEmpty(et))
                {
                    wsretlog = "日期为空";
                    errtype = 3;
                    MySqlHelper.ExecuteNonQuery(System.Data.CommandType.Text, string.Format("UPDATE FLOW_RUN set TIMES=TIMES+1,RETRY=0 where RUN_ID={0} ", flow149[i].run_id), null);
                    MySqlHelper.ExecuteNonQuery(System.Data.CommandType.Text, string.Format("INSERT INTO sync_flowlog (run_id,flow_id,run_name,sendlog,receivelog,sendtime,errtype) values ({0},{1},'{2}','{3}','{4}',sysdate(),{5}) ", flow149[i].run_id, flowid, flow149[i].run_name, "", wsretlog, errtype), null);
                    continue;
                }
                if (!string.IsNullOrEmpty(st) && !string.IsNullOrEmpty(et))
                {
                    ...
                    if (endtime.CompareTo(starttime) > 0)
                    {
                        ...
                        string versl = " ";
                        if (flow149[i].i_versl.ToString() == "加班结薪")
                        {
                            versl = "1";
                        }
                        else if (flow149[i].i_versl.ToString() == "调休")
                        {
                            versl = " ";
                        }
                        else
                        ...
                        oasaphead17.I_PERNR = flow149[i].user_id.ToString();
                        ...
                        try
                        {
                            ...
                            Logger.Log(flow149[i].run_id + "(" + flow149[i].run_name + "-" + flow149[i].user_id + "):" + JsonHelper.ObjectToJson(oasap17), flowid.ToString());
                            WebReference.DT_OA_OA17_RespITEM[] retoa = oa.SAP_OA_JK_17(oasap17);
                            wsretlog = JsonHelper.ObjectToJson(retoa);
                            Logger.Log(flow149[i].run_id + "(" + flow149[i].run_name + "-" + flow149[i].user_id + "):" + JsonHelper.ObjectToJson(retoa), flowid.ToString());
                        ...
                        MySqlHelper.ExecuteNonQuery(System.Data.CommandType.Text, string.Format("UPDATE FLOW_RUN set TIMES=TIMES+1,RETRY=0  where RUN_ID={0} ", flow149[i].run_id), null);
                        MySqlHelper.ExecuteNonQuery(System.Data.CommandType.Text, string.Format(" INSERT INTO sync_flowlog (run_id,flow_id,run_name,sendlog,receivelog,sendtime,errtype) values ({0},{1},'{2}','{3}','{4}',sysdate(),{5})", flow149[i].run_id, flowid, flow149[i].run_name, JsonHelper.ObjectToJson(oasap17), wsretlog, errtype), null);
                        if (ifsuc)
                        {
                            int ret = MySqlHelper.ExecuteNonQuery(System.Data.CommandType.Text, string.Format("update flow_run set SYNC_TIME=sysdate() ,RETRY=0 where RUN_ID={0} ", flow149[i].run_id), null);
                            //更新数据库表示他已经同步过了并且同步成功了
                            if (ret > 0)
                                Logger.Log(string.Format("RUN_ID={0},已经更新数据库", flow149[i].run_id), flowid.ToString());
                            else
                                Logger.Error(string.Format("RUN_ID={0},更新没有成功", flow149[i].run_id), flowid.ToString());
                        }
                        else
                        {
                            Logger.Log(string.Format("RUN_ID={0},SAP返回错误信息", flow149[i].run_id), flowid.ToString());
                        }
                    }
                }
            }
