    ...
    public class Timer181
    {
        ...
        public void Start(object source, System.Timers.ElapsedEventArgs e)
        {
            ...
            for (int i = 0; i < flow181.Count; i++)
            {
                ...
                MySqlHelper.ExecuteNonQuery(System.Data.CommandType.Text, string.Format("UPDATE FLOW_RUN set TIMES=TIMES+1 where RUN_ID={0} ", flow181[i].run_id), null);
                MySqlHelper.ExecuteNonQuery(System.Data.CommandType.Text, string.Format("INSERT INTO sync_flowlog (run_id,flow_id,run_name,sendlog,receivelog,sendtime) values ({0},{1},'{2}','{3}','{4}',sysdate())", flow181[i].run_id, flowid, flow181[i].run_name, JsonHelper.ObjectToJson(oasap15), retoastring), null);
                if (ifsuc)
                {
                    //这里改版 oaphp加页面 加表 3次 3次以上 点重新发送 retry重置为1
                    int ret = MySqlHelper.ExecuteNonQuery(System.Data.CommandType.Text, string.Format("update flow_run set SYNC_TIME=sysdate() where RUN_ID={0} ", flow181[i].run_id), null);
